...
function resolveStyleHref(path: string): string {
  try {
    const runtime = (chrome as unknown as { runtime?: { getURL?: (input: string) => string } }).runtime;
...
function ensureDocumentStylesheet(doc: Document, id: string, path: string): void {
...
function ensureShadowStylesheet(shadowRoot: ShadowRoot, id: string, path: string): void {
...
export function ensureShadowUiBaseStyles(shadowRoot: ShadowRoot): void {
  ensureShadowStylesheet(shadowRoot, TOKEN_PRIMITIVES_ID, TOKEN_PRIMITIVES_PATH);
